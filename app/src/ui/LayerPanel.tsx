  ...
  return (
    <aside className="bg-panel border-r border-line w-56 flex flex-col select-text">
      <div className="px-3 py-2 flex items-center justify-between border-b border-line">
        <div className="text-[10px] font-bold tracking-widest text-muted uppercase">Layers</div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar">
        {layers.map((l) => (
          ...
          >
            ...
            />
            <button
              onClick={(e) => { e.stopPropagation(); if (confirm(`Delete layer "${l.name}" and its entities?`)) store.removeLayer(l.id); }}
              title="Delete layer"
